...
gtFiles = dir(fullfile(gtFolder, '*.gif'));

sensitivityValues = []; % Initialize as empty array
falsePositiveRateValues = []; % Initialize as empty array
...
for i = 1:length(imageFiles)
    ...
    [binary_MF_FDOG_closed, resultsTable] = processImage(I_Original, GT);

    sensitivityValues(end+1) = resultsTable.Sensitivity;
    falsePositiveRateValues(end+1) = 1 - resultsTable.Specificity;

end

% Sort the points by false positive rate
[sortedFPR, sortIndex] = sort(falsePositiveRateValues);
sortedTPR = sensitivityValues(sortIndex);

% Plot the sorted points and connect them with lines
figure;
plot(sortedFPR, sortedTPR, '-o');
xlabel('False Positive Rate (1 - Specificity)');
ylabel('True Positive Rate (Sensitivity)');
title('ROC Space of MF-FDOG for the Dataset');
xlim([0, 0.05]); % Limit the x-axis to false positive rates < 0.05

function [binary_MF_FDOG_closed, resultsTable] = processImage(I_Original, GT)
        % Read Train Image
    ...
    [resultsTable] = ground_truth(binary_MF_FDOG_closed, GT);
...
function [resultsTable] = ground_truth(binary_MF_FDOG_closed, GT)
