...
function f = ground_truth(binary_MF_FDOG)
    ...
    TP = sum(sum(GT_binary & binary_MF_FDOG)); % True Positives
    TN = sum(sum(~GT_binary & ~binary_MF_FDOG)); % True Negatives
    FP = sum(sum(~GT_binary & binary_MF_FDOG)); % False Positives
    FN = sum(sum(GT_binary & ~binary_MF_FDOG)); % False Negatives
